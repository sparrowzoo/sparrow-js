...
import {WebSocketContext, WebSocketContextValue,} from "@/lib/im/WebSocketProvider";
        ...
        crosStorage
            ...
            .then((token) => {
                ...
                crosStorage?.locateToken(token).then((token) => {
                    ...
                    messageBroker.webSocket.handshakeFail = (data: Result) => {
                        //鉴权失败，则重连
                        crosStorage
                            .getToken(StorageType.AUTOMATIC, ChatApi.getVisitorToken)
                            .then((token) => {
                                console.log("token is ready  ", token);
                                messageBroker.webSocket.reconnectWebSocket();
                            });
                    };
