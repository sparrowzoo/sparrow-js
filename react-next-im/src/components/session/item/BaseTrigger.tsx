...
export default function BaseTrigger(sessionItemProps: SessionItemProps) {
  ...
  return (
    <div className="im-session-preview flex flex-row h-10 w-full">
      <MyAvatar
      ...
      <div
        className={
          "im-session-preview-body flex flex-1 flex-col justify-start items-center ml-2 text-xs"
        }
      >
        <div className={"flex flex-row justify-between items-between w-full"}>
          <span className="im-session-preview-name">{chatSession.name}</span>
          <span className="im-session-preview-time">{chatSession.lastMessage?.sendTime}</span>
        </div>
        <div className={"flex flex-row justify-between items-between w-full"}>
          <span
            title={chatSession.lastMessage?.content}
            className={
              "im-session-preview-message text-gray-400 flex-1 text-left text-xs truncate  w-[10rem]"
            }
          >
          ...
          {chatSession.unreadCount > 0 &&
            sessionItemProps.unreadPosition == Position.TAIL && (
              <Badge
                className={
                  "im-session-unread bg-red-500 p-1 w-5 h-5 text-xs text-center  text-white z-50 font-bold rounded-full"
                }
              >
