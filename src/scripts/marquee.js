...
Marquee.prototype.Start = function() {
	...
	msobj.Begin = function() {
		msobj.ClientScroll = msobj.Direction > 1 ? msobj.ID.scrollWidth / 2
		...
		if (msobj.ScrollStep < 0)
			return;
		msobj.ID.onmousemove = function(e) {
			if (msobj.ScrollStep == 0 && msobj.Direction > 1) {
				e = e || window.event;
				if (window.event) {
					if (msobj.IsNotOpera) {
						msobj.EventLeft = e.srcElement.id == msobj.ID.id ? e.offsetX
								- msobj.ID.scrollLeft
								: event.srcElement.offsetLeft
										- msobj.ID.scrollLeft + e.offsetX;
					} else {
