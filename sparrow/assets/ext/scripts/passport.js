Sparrow.passport = {
  ...
  bindEvents: function () {
    ...
    $(document).enter(function () {
      var submitButton =
        $("divShortCutRegister").className == "block"
          ? $("btnRegister")
          : $("btnLogin");
  ...
  authenticationCallback: function (result) {
    ...
    var days =
      chkLoginDays != null && chkLoginDays.checked
        ? parseInt(chbLoginDays.value) * 24 * 60 * 60
        : 0;
    $.browser.setCookie(
      $.browser.cookie.permission,
      result.data.token,
      days,
      $.browser.cookie.root_domain
    );
    /* 回调函数的名字空间 */
    ...
    /* 注册成功后默认登录状态 */
    //Sparrow.thread.like.loginCallback(args);
    var callbackScript =
      "parent.$" + nsOfCallback + ".loginCallback('" + callBackParameter + "')";
    console.log("login callback script {}", callbackScript);
    eval(callbackScript);
  ...
  register: {
    validate: {
      email: function (srcElement) {
        ...
        $.http.post(
          $.url.root + "/user/validate-email.json",
      ...
      user: function (srcElement) {
        ...
        $.http.post(
          $.url.root + "/user/validate-user-name.json",
    ...
    activate: function () {
      ...
      $.http.post($.url.passport + $.passport.api.activate, "email=" + email);
    },
  },
  login: {
    /* 快捷登录事件 */
