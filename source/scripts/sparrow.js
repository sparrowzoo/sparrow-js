...
(function (global, factory) {
  ...
  var Sparrow = function (selector, parent, doc, cache, sparrowContainerKey) {
    if (window === this) {
      ...
      if (
        ...
        beginSelector.indexOf(selector.substring(0, 1)) == -1
      ) {
        //btnButtonId
        ...
        case "+":
          ...
          if (selectorArray.length >= 3) {
            if (selectorArray[2] === "doc") {
              ...
              this.doc
                .getElementById(selectorArray[2])
            ...
            if (tagArray[i].checked) {
              if (attribute) {
                selectedTag[selectedTag.length] =
                  tagArray[i].attributes[attribute].value;
              } else {
  ...
  Sparrow.browser = {
    ...
    cookie: {
      ...
      root_domain: $(function () {
        return window.location.host.substr(
    ...
    version: $(function () {
      if (navigator.userAgent.search(/MSIE/gim) !== -1) {
        ...
        return RegExp.$1;
      } else if (navigator.userAgent.search(/Opera/gim) !== -1) {
        navigator.userAgent.match(/Version\/([0-9\.]+)/gim);
        return RegExp.$1;
      } else if (navigator.userAgent.search(/Firefox/gim) !== -1) {
        navigator.userAgent.match(/Firefox\/([0-9\.]+)/gim);
        return RegExp.$1;
      } else if (navigator.userAgent.search(/Chrome/gim) !== -1) {
 ...
 */
  Sparrow.v = {
    ...
    getErrorLabel: function (validate) {
      return validate.errorCtrlId
        ? $(validate.errorCtrlId.join($.v.index))
        : null;
    },
    ...
    _setBackground: function (validate, color, srcElement) {
      ...
      if (parentLevel > 0) {
        ...
        try {
          while (
            background.tagName.toUpperCase() !== "TR" &&
            background.className !== "line" &&
            background.className !== "validate"
          ) {
    ...
    _validate: function (validate, srcElement) {
      ...
      var errorCtrl = this.getErrorLabel(validate);
      var length =
        srcElement.tagName.toUpperCase() === "SELECT" &&
        srcElement.multiple === true
          ? srcElement.options.length
      ...
      // 长度不合法
      if (
        (validate.maxLength && length > validate.maxLength) ||
        (validate.minLength && length < validate.minLength)
      ) {
      ...
      if (
        errorCtrl &&
        errorCtrl.className === "error" &&
        errorCtrl.innerHTML === "!" + validate.setError
      ) {
    ...
    isIdCard: function (validate, srcElement) {
      ...
      if (
        srcElement.value.search(/^([1-9]([0-9]{16}|[0-9]{13})([0-9]|x|X))$/) ===
        -1
      ) {
    ...
    isEqual: function (validate, srcElement) {
      ...
      if (
        srcElement.value !==
        $(validate.otherCtrlId.join($.v.index)).value.trim()
      ) {
    ...
    isDigital: function (validate, srcElement) {
      ...
      if (
        (defMin && floatValue < validate.minValue) ||
        (defMax && floatValue > validate.maxValue)
      ) {
        return this.fail(validate, validate.digitalError);
      }
      this.ok(validate, srcElement);
    ...
    isFileLength: function (srcElement, defaultValue) {
      if (srcElement.value.toUpperCase().indexOf("M") !== -1) {
        if (
          ...
          srcElement.value =
            srcElement.value.toUpperCase().split("M")[0] + "MB";
        } else {
          srcElement.value = defaultValue;
        }
        ...
        if (
          ...
          srcElement.value =
            srcElement.value.toUpperCase().split("K")[0] + "KB";
        }
        srcElement.value = defaultValue;
      } else {
        srcElement.value = defaultValue;
  ...
  Sparrow.page = {
    toTargetPage: function (pageCount, pageFormat, srcElement) {
      ...
      var currentPageIndex = parseInt(
        $("spanCurrentPageIndex").innerHTML.trim()
      );
      ...
      window.location.href = pageFormat.replace(
        "$pageIndex",
        consumerPageIndex
      );
    },
  ...
  Sparrow.isNullOrEmpty = function (sourceString) {
    return (
      sourceString == null ||
      typeof sourceString === "undefined" ||
      (typeof sourceString === "string" &&
        (sourceString.trim() === "" || sourceString.trim() === "null"))
    );
  };
  ...
  Sparrow.getBeforeTime = function (updateTime) {
    ...
    return $(
      function (timeSplit, depth) {
        if (timeSplit / beforeDateConfig[depth].split < 1) {
          // 本地时间与服务器时间不一致导致出现负数
          return (
            (parseInt(timeSplit) < 0 ? 1 : parseInt(timeSplit)) +
            beforeDateConfig[depth].name
          );
        } else {
          return arguments.callee(
            timeSplit / beforeDateConfig[depth].split,
            ++depth
          );
        }
      },
      (currentTime - updateTime) / 1000,
      0
    );
  };
  ...
  Sparrow.clearForm = function (validateJson) {
    ...
    for (var i = 0; i < inputArray.length; i++) {
      ...
      var defaultValue = "";
      if (validateJson[inputArray[i]])
        defaultValue = validateJson[inputArray[i]].defaultValue;
  ...
  Sparrow.prototype.css = function (attribute, value, add) {
    ...
    if (attribute === "opacity") {
      ...
      if (add) {
        command =
          'var o=parseInt($("' + this.s.id + '").style.' + attribute + ",10);";
        command +=
          '$("' +
          this.s.id +
          '").style.' +
        ...
        command =
          '$("' + this.s.id + '").style.' + attribute + '="' + value + '"';
      }
      eval(command);
    }
    return this;
  };
  Sparrow.prototype.change_check_box_class = function (
    checked,
    index,
    key,
    split
  ...
  Sparrow.prototype.center = function () {
    ...
    var wordWindowHeight =
      this.doc.documentElement.clientHeight > screen.availHeight
        ? screen.availHeight
  ...
  Sparrow.win = {
    config: {
      ...
      closeBtnImg:
        $.url.resource + "/images/" + $.website.themes + "/panel.gif",
    ...
    addPanel: function (url) {
      ...
      panel.s.zIndex = 1001;
      panel.s.style.cssText =
        "position:absolute;text-align:center;font-size: 10pt;background:white;";
      ...
      frame.s.src = url;
      frame.loadComplete(function (element) {
        var width = parseInt(
          element.contentWindow.document.body.style.width,
          10
        );
        var height = parseInt(
        ...
        frame.s.style.height =
          height -
          ($.win.config.showHead ? $.win.config.titleHeight : 0) -
          5 +
          "px";
        ...
        if ($.win.config.showHead !== false) {
          $("#divleft", null, panel.doc).s.innerHTML =
            element.contentWindow.document.title;
        }
      });
    },
    // 加标题
    ...
    okClick: function () {},
    closeClick: function () {
      $.showOrHiddenTag(this.config.tagArray, true, this.getWindow().document);
  ...
  Sparrow.alert = function (msg, type, title, url, wait_message) {
    ...
    if (typeimg) {
      content =
        "<br/>" + '<img align="absMiddle" src=\'' + typeimg + "'/>&nbsp;&nbsp;";
    }
    if (msg) {
      content += msg;
    ...
    $.showOrHiddenTag(
      "select",
      true,
      $.win.getWindow().document.getElementById("divcontent")
    );
  ...
  Sparrow.window = function (config) {
    ...
    if (config.srcElement) {
      ...
      $.win.config.toTopHeight =
        $(config.srcElement).getAbsoluteTop() - height / 2;
    }
    // 背景层
    $.win.addBackDiv();
    $.win.addPanel(url);
  ...
  Sparrow.dialog = function (config) {
    ...
    $.win.addClose();
    if (config.content) {
      $("#divcontent", null, $.win.getWindow().document).s.innerHTML =
        config.content;
    }
  ...
  Sparrow.gridView = {
    ...
    submit: function (postUrl, confirmMessage, isOnlyOne) {
      var result = isOnlyOne
        ? this.onlyCheckedOne(confirmMessage)
        : this.mustSelect(confirmMessage);
      if (!result) {
        return;
      }
    ...
    updateRow: function (rowIndex) {
      for (var i = 0; i < this.tr[0].td.length; i++) {
        if (this.tr[0].td[i].innerHTML) {
          this.body.rows[rowIndex].cells[i].innerHTML =
            this.tr[0].td[i].innerHTML;
        }
      }
    },
    ...
    uploadDelegate: function (key, editor, srcElement) {
      ...
      if ($.file.isShowProgress !== false) {
        ...
        if (!$("divStatus")) {
          ...
          divStatus.s.style.top =
            sparrowUploadFrame.getAbsoluteTop() - 10 + "px";
  ...
  Sparrow.event.prototype = {
    ...
    move: function () {
      if (this.dragapproved && this.srcElement != null) {
        this.srcElement.style.left =
          this.srcLeftPos + this.clientX - this.eventX;
        this.srcElement.style.top =
          this.srcRightPos + this.clientY - this.eventY;
        return;
      }
      return true;
    },
  ...
  Sparrow.prototype.addItem = function (text, value) {
    ...
    if (!this.existItem(newoption)) this.s.options.add(newoption);
  };
  Sparrow.prototype.existItem = function (srcOption) {
    var flag = false;
    for (var j = 0; j < this.s.options.length; j++) {
      if (srcOption.value === this.s.options[j].value) flag = true;
    }
    return flag;
  };
  ...
  Sparrow.prototype.upDown = function (direction) {
    if (this.s.selectedIndex < 0) return;
    if (direction < 0) {
      if (this.s.selectedIndex === 0) return;
    } else {
      if (this.s.selectedIndex === this.s.options.length - 1) return;
    }
  ...
  Sparrow.message = function (content, srcElement) {
    ...
    divmsg = $("+div." + id);
    divmsg.s.style.cssText =
      "position:absolute;background-color:#cccccc;width:auto;padding:10px;text-align:left;";
    ...
    if (srcElement) {
      ...
      divmsg.s.style.top =
        sparrowElement.getAbsoluteTop() - divmsg.s.offsetHeight + "px";
      divmsg.s.style.left =
        sparrowElement.getAbsoluteLeft() -
        (divmsg.s.offsetWidth - srcElement.offsetWidth) / 2 +
        "px";
    } else {
      divmsg.center();
    }
  ...
  Sparrow.menu.prototype.side = function () {
    ...
    var menuHTML = [];
    menuHTML.push(
      '<ul style="{0}">'.format($.css.menu.ul.format(this.config.width - 2))
    );
    for (var i = 0; i < this.config.menu.length; i++) {
      menuHTML.push(
        '<li style="{0}" {3}><a href="javascript:void(0);" onclick="{1}.itemClick({2});"  style="width:{4}px;display:inline-block;cursor:pointer"><span style="float:left">{5}</span><span  style="float:right;">{6}</span></a></li>'.format(
          $.css.menu.li.format(this.config.width),
          this.obj,
          i,
          this.config.menu[i].more
            ? 'onmouseover="{0}.itemMore(this,{1});"'.format(this.obj, i)
            : "",
          this.config.width,
  ...
  Sparrow.menu.prototype.vertical = function () {
    ...
    item.each(function (i) {
      ...
      if (child) {
        ...
        $(item_link).bind("onclick", function (e) {
          $.event(e).cancelBubble();
          ...
          if (child.s.style.display === "block") {
            ...
            child.animation(
              "{height:'" + menu.config.position[child.s.id] + "px'}",
              menu.config.period
            );
          }
        });
  ...
  Sparrow.menu.prototype.hidden = function () {
    ...
    if (this.position === $.HORIZONTAL) {
      ...
      $(this.config.current_menu.parentNode).animation(
        "{height:'0px'}",
        this.config.period
      );
    }
  ...
  Sparrow.menu.prototype.show = function (srcElement, parentMenu) {
    ...
    var scrollTop = 0;
    if (this.config.container) scrollTop = this.config.container.scrollTop;
    var left = $(this.config.srcElement).getAbsoluteLeft();
    if (this.config.position === this.SIDE) {
      left += this.config.srcElement.offsetWidth;
    }
    var top = $(this.config.srcElement).getAbsoluteTop() - scrollTop;
  ...
  Sparrow.menu.prototype.horizontal = function () {
    ...
    this.config.menu.each(function (i) {
      this.id = menu.obj + "_" + menu.position + "_menu_" + i;
      //初始化list的位置和事件
      var list = $(menu.config.list[i]);
      list.s.style.cssText = "height:{0}px;overflow:hidden;width:{1}px;".format(
        list.s.offsetHeight,
        list.s.offsetWidth + 10
      ); //加2误差
      ...
      $(this).bind("onmouseover", function (e) {
        ...
        list.parentNode.style.top =
          top + menu.config.menu[0].offsetHeight - 8 + "px";
        if (list.getElementsByTagName("li").length > 0) {
          ...
          var left =
            parseInt(e.srcElement.offsetLeft, 10) -
          ...
          $(list.parentNode).animation(
            "{height:'" + menu.config.position["height"] + "px'}",
            menu.config.period
          );
        }
      });
  ...
  Sparrow.datePicker = function (pickerId) {
    ...
    dateFormat["yyyy年MM月dd日"] = new RegExp(
      "^(\\d{4})年(\\d{2})月(\\d{2})日$",
      "ig"
    );
    ...
    this.config = {
      ...
      maxDaysOfMonth: [31, 28, 31, 30, 31, 30, 31, 31, 30, 31, 30, 31],
  ...
  Sparrow.datePicker.prototype.init = function (yyyy, MM, dd) {
    ...
    if (!this.pickerDiv) {
      ...
      this.pickerDiv.s.style.cssText =
        "position:absolute;display:none;text-align:center";
      this.pickerDiv.s.style.left = sparrowElement.getAbsoluteLeft() + "px";
      this.pickerDiv.s.style.top =
        sparrowElement.getAbsoluteTop() +
        this.config.srcElement.clientHeight +
        "px";
      ...
      var events =
        ...
        ".show();};" +
        this.obj +
        ".config.srcElement.onchange=function(){" +
        this.obj +
        ".validate();" +
        this.obj +
        ".init();};";
    ...
    var startDayOfPreMonth =
      maxDaysOfPreMonth - (startDayOfMonth === 0 ? 7 : startDayOfMonth) + 1;
    var datePickerHTML = [];
    datePickerHTML.push('<table class="pure-table pure-table-bordered">');
    datePickerHTML.push("<tr>");
    datePickerHTML.push(
      '<td><a  href="javascript:void(0);" onclick="{0}.changeMonth(-1)">&lt;<a></td>'.format(
        this.obj
      )
    );
    datePickerHTML.push(
      '<td colspan="3"><a onclick="{0}.initYear({1});" href="javascript:void(0)">{1}</a>年</td>'.format(
        this.obj,
        yyyy
      )
    );
    datePickerHTML.push(
      '<td colspan="2"><a onclick="{0}.initMonth({2},{3});" href="javascript:void(0);">{1}</a></td>'.format(
        this.obj,
        this.config.month[MM],
        yyyy,
        MM
      )
    );
    ...
    datePickerHTML.push("<tr>");
    for (
      var dayIndexOfPreMonth = startDayOfPreMonth;
      dayIndexOfPreMonth <= maxDaysOfPreMonth;
      dayIndexOfPreMonth += 1
    ) {
      datePickerHTML.push(
        '<td><a  style="color:#ccc;">{0}</a></td>'.format(dayIndexOfPreMonth)
      );
      tdIndex += 1;
      if (tdIndex % 7 === 0) {
        datePickerHTML.push("</tr>");
    ...
    for (
      var dayIndexOfMonth = 1;
      dayIndexOfMonth <= maxDaysOfMonth;
      dayIndexOfMonth += 1
    ) {
      datePickerHTML.push(
        '<td><a href="javascript:void(0);" onclick="{0}.changeDate({1},{2},{3});">{3}</a></td>'.format(
          this.obj,
          yyyy,
          MM,
          dayIndexOfMonth
    ...
    for (
      var dayIndexOfNextMonth = 1;
      dayIndexOfNextMonth <= daysOfNextMonth;
      dayIndexOfNextMonth += 1
    ) {
      datePickerHTML.push(
        '<td><a  style="color:#ccc;">{0}</a></td>'.format(dayIndexOfNextMonth)
      );
      tdIndex += 1;
      if (tdIndex % 7 === 0) {
        datePickerHTML.push("</tr>");
  ...
  Sparrow.datePicker.prototype.initYear = function (yyyy) {
    ...
    datePickerHTML.push("<tr>");
    datePickerHTML.push(
      '<td><a href="javascript:void(0);" onclick="{0}.initYear({1})">&lt;<a></td>'.format(
        this.obj,
        yyyy - 10
      )
    );
    datePickerHTML.push(
      '<td colspan="2">{0}-{1}</td>'.format(startYear, endYear - 1)
    );
    datePickerHTML.push(
      '<td><a href="javascript:void(0);" onclick="{0}.initYear({1})">&gt;<a></td>'.format(
        this.obj,
        yyyy < 1900 ? 1910 : yyyy + 10
      )
    );
    datePickerHTML.push("</tr>");
    datePickerHTML.push("<tr>");
    ...
    for (var i = startYear - 1; i < endYear; i += 1) {
      if (i === startYear - 1 || i === endYear) {
        ...
        datePickerHTML.push(
          '<td><a href="javascript:void(0);" onclick="{0}.initMonth({1})">{1}</a></td>'.format(
            this.obj,
            i
          )
        );
  ...
  Sparrow.datePicker.prototype.initMonth = function (yyyy, MM) {
    ...
    datePickerHTML.push("<tr>");
    datePickerHTML.push(
      '<td><a href="javascript:void(0);" onclick="{0}.initMonth({1},{2})">&lt;<a></td>'.format(
        this.obj,
        yyyy - 1,
        MM
      )
    ...
    datePickerHTML.push(
      '<td style="text-align:center;" colspan="2"><a href="javascript:void(0);" onclick="{0}.initYear({1})">{1}</a></td>'.format(
        this.obj,
        yyyy
      )
    );
    ...
    for (var i = 0; i < 12; i += 1) {
      datePickerHTML.push(
        '<td><a href="javascript:void(0);" onclick="{0}.init({1},{2},{3})">{4}</a></td>'.format(
          this.obj,
          yyyy,
          i,
  ...
  Sparrow.datePicker.prototype.changeMonth = function (direction) {
    ...
    this.config.srcElement.value = this.getFormatDate(
      currentYear,
      currentMonth,
      currentDay
    );
  ...
  Sparrow.datePicker.prototype.validate = function (yyyy, MM, dd) {
    ...
    if (
      this.config.srcElement.value.search(
        this.config.format[this.config.currentFMT]
      ) === -1
    ) {
      $.message(
        "请按【" + this.config.currentFMT + "】格式输入",
        this.config.srcElement
      );
      return false;
    }
    ...
    if (dd < 0 || dd > this.getMaxDaysOfMonth(yyyy, MM)) {
    ...
    if (this.config.minDate != null) {
      var minDate = new Date(
        this.config.minDate.getFullYear(),
        this.config.minDate.getMonth(),
        this.config.minDate.getDate()
      );
  ...
  Sparrow.editor = function (objName, parentName) {
    ...
    this.config = {
      ...
      tool: {
        ...
        toolBar: [
          ...
          /* 8 */ {
            split:
              '<img src="{0}icoBack.gif" style="background:url({0}ico.gif) -940px -8px;width:8px;height:24px;"/>',
          },
          /* 9 */ {
            left: -144,
            top: -8,
          ...
          /* 13 */ {
            split:
              '<img alt="" src="{0}icoBack.gif" style="background:url({0}ico.gif)-940px -8px;width:8px;height:24px;"/>',
          },
          /* 14 */ {
            left: -562,
            top: -8,
        ...
        face: [
          {
            url: $.url.resource + "/images/sparrowEditor/face/01.gif",
            name: "呲牙",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/02.gif",
            name: "不嘛",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/03.gif",
            name: "哭泣",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/04.gif",
            name: "嘟嘟",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/05.gif",
            name: "嗯嗯",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/06.gif",
            name: "思考下",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/07.gif",
            name: "翻跟斗",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/08.gif",
            name: "我靠",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/09.gif",
            name: "扫射",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/10.gif",
            name: "嘘!",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/11.gif",
            name: "害羞",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/12.gif",
            name: "眯眯",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/13.gif",
            name: "求保佑",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/14.gif",
            name: "晕",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/15.gif",
            name: "来一拳",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/16.gif",
            name: "胜利",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/17.gif",
            name: "嚎叫",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/18.gif",
            name: "BIBI",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/19.gif",
            name: "哼哼",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/20.gif",
            name: "冷汗",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/21.gif",
            name: "泼墨",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/22.gif",
            name: "吐血",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/23.gif",
            name: "扣鼻子",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/24.gif",
            name: "挠挠脸",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/25.gif",
            name: "舔手指",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/26.gif",
            name: "吃饭啦",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/27.gif",
            name: "打哈欠",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/28.gif",
            name: "拜拜",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/29.gif",
            name: "抽一口",
          },
          {
            url: $.url.resource + "/images/sparrowEditor/face/30.gif",
            name: "杀死",
          },
        ],
      },
    ...
    this.attach = {
      ...
      loadAttach: function () {
        ...
        //初始化图片的tabs
        $("#tab" + editor.obj).tabs();
        var uploadedFileList = editor.config.attach.uploadedJson.json();
        // 如果有已经上传的文件
            ...
            if (imgContainer.innerHTML.trim() === "") {
              for (var i = uploadedFileList.length - 1; i >= 0; i--) {
                ...
                imgContainer.appendChild(
                  getImgContainer(fileUrl, clientFileName, editor)
                );
              }
            }
          }
      ...
      collectFilePostInfo: function () {
        ...
        if (attachTable) {
          // 获取当前编辑器的文件id
          var fileUuidArray = $("&" + editor.config.attach.fileUUID);
          // 获取当前编辑器的文件备注信息
          var fileRemarkArray = $("&" + editor.config.attach.fileRemark);
          var fileInfo = [];
          for (var i = 0; i < fileUuidArray.length; i++) {
            if (fileUuidArray[i].value) {
              fileInfo.push(
                fileUuidArray[i].value + ":" + fileRemarkArray[i].value
              );
            }
          }
      ...
      _submit: function () {
        // 收集需要上传的文件信息
        $(this.parentObject.config.attach.fileInfoId).value =
          this.parentObject.attach.collectFilePostInfo();
        ...
        if (this.parentObject.config.attach.currentUUID) {
          $.submit("update.do", this.parentObject.config.attach.formIndex);
          return;
        }
      ...
      submit: function (validateConfig, nsOfLoginCallback) {
        // 获取当前编辑器
        var editor = this.parentObject;
        $(editor.config.contentCtrlId).value =
          editor.frame.contentWindow.document.body.innerHTML;
      ...
      insertEditor: function (toolip, serverFileName) {
        var editor = this.parentObject;
        var insertHtml =
          '<img title="' + toolip + '"' + ' src="' + serverFileName + '"/>';
      ...
      deleteOnServer: function (fileId, srcElement) {
        ...
        if (window.confirm($.message.deleteFile)) {
          $.ajax.json(
            $.url.root + "/attach/delete.json",
            "uuid=" + fileUUID,
            function (result) {
              editor.attach.deleteImg($.ajax.srcElement);
              $.message("删除成功！");
            },
            srcElement
          );
  ...
  Sparrow.editor.prototype.getTempNode = function (tagName) {
    ...
    // execCommand情况
      ...
      var nodeArray =
        this.frame.contentWindow.document.getElementsByTagName(tagName);
      for (var i = nodeArray.length - 1; i >= 0; i--) {
        ...
        if (
          tempAttributeValue &&
          tempAttributeValue.indexOf(this.config.tempNodeAttribute) !== -1
        ) {
  ...
  Sparrow.editor.prototype.getBrief = function () {
    var brief = null;
    if ($.browser.ie) {
      brief = this.frame.contentWindow.document.body.innerText.substring(
        0,
        300
      );
    } else {
  ...
  Sparrow.editor.prototype.getEditorContent = function () {
    if (this.config.tool.convertHTML.isConvert) {
      var tdconvertHTML = document.getElementById(
        this.config.tool.convertHTML.ctrlId
      );
      if (tdconvertHTML.innerHTML !== "HTML") {
        tdconvertHTML.innerHTML = "HTML";
        document.getElementById(
          this.config.tool.icon.containerId
        ).style.display = "block";
        if ($.browser.ie) {
          this.frame.contentWindow.document.body.innerHTML =
            this.frame.contentWindow.document.body.innerText;
        } else {
          this.frame.contentWindow.document.body.innerHTML =
            this.frame.contentWindow.document.body.textContent;
        }
      }
    }
    this.clear();
  ...
  Sparrow.editor.prototype.getRange = function () {
    this.frame.contentWindow.document.body.focus();
    return $.browser.ie
      ? this.frame.contentWindow.document.selection.createRange()
  ...
  Sparrow.editor.prototype.m_down = function (srcObj) {
    ...
    srcObj.style.background =
      "url(" +
      this.config.tool.icon.path +
      "ico.gif) " +
      (this.config.tool.toolBar[key].left + 1) +
      "px " +
      (this.config.tool.toolBar[key].top + 1) +
      "px";
  };
  Sparrow.editor.prototype.m_up = function (srcObj) {
    ...
    srcObj.style.background =
      ...
      this.config.tool.icon.path +
      "ico.gif) " +
      this.config.tool.toolBar[key].left +
      "px " +
      this.config.tool.toolBar[key].top +
      "px";
    this.run(srcObj.id);
  };
    ...
    try {
      ...
      if ($.browser.ie) {
        ...
        if (range.startContainer.getElementsByTagName) {
          var childNodes = range.startContainer.getElementsByTagName(tagName);
  ...
  Sparrow.editor.prototype.run = function (srcElementId) {
    ...
    if (
      ...
      this.frame.contentWindow.document.execCommand(
        this.config.tool.toolBar[key].cmd,
        false,
        undefined
      );
    }
  ...
  Sparrow.editor.prototype.show = function (srcObject, key, width, maxHeight) {
    ...
    var htmlFrameId =
      this.obj + "_" + this.config.tool.toolBar[key].htmlFrameId;
    ...
    var leftPosition =
      sparrowObject.getAbsoluteLeft() - (width - srcObject.offsetWidth) / 2;
    var left = $(srcObject.parentNode).getAbsoluteLeft();
    if (leftPosition < left) leftPosition = left;
    listDiv.style.left = leftPosition + "px";
    listDiv.style.top =
      sparrowObject.getAbsoluteTop() + srcObject.offsetHeight + "px";
    this.config.interval = window.setInterval(
      this.fullObjName + ".intervalShow(" + key + "," + maxHeight + ")",
      10
    );
  };
  ...
  Sparrow.editor.prototype.getHtml = function (key) {
    ...
    switch (key) {
      ...
      case 1:
        HTML.push(
          '<ul style="line-height:25px; list-style-type:none;margin:0;padding:0;">'
        );
      ...
      case 20:
        ...
        HTML.push(
          '<label>\u94fe\u63a5\u5730\u5740</label><input class="pure-input-1" onkeyup="if(this.value.substring(0,7)!=\'http://\'){this.value=\'http://\'}" value="http://' +
            url +
            '" type="text" id="' +
            this.obj +
        ...
        for (i = 0; i < row; i++) {
          ...
          for (j = 0; j < col; j++) {
            if (index < this.config.tool.face.length) {
              HTML.push(
                '<td><img style="cursor: pointer;" title="' +
                  this.config.tool.face[index].name +
                  '" src="' +
                  this.config.tool.face[index].url +
                  '" onclick="' +
                  this.fullObjName +
                  '.callBackRun(22,event);" /></td>'
              );
            } else {
              HTML.push("<td></td>");
            }
      ...
      case 22:
        this.createTempNode("span");
        HTML.push(
          '<label>FLASH视频URL</label><input class="pure-input-rounded" id="' +
            this.obj +
            '_txtVideo" type="text"/><br/><span id="' +
            this.obj +
            '_spanVideoErrorMessage"></span><br/>'
        );
        HTML.push(
          '<input value="插入视频" class="pure-button pure-button-primary" type="button" onclick="' +
            this.fullObjName +
      ...
      case 23:
        ...
        HTML.push(
          '<a target="_self" href="javascript:void(0);" class="pure-menu-link">'
        );
        ...
        HTML.push(
          '<div id="' +
            this.config.attach.key +
            this.config.attach.localUploadImgTabId +
            '" class="block">'
        );
        HTML.push(
          '<div style="width:100%;height:auto;overflow:hidden;padding:2px;" id="' +
            this.config.attach.uploadImgContainerId +
            '"></div>'
        );
        HTML.push(
          '<iframe name="' +
            this.config.attach.iframeName +
            '" id="' +
        ...
        HTML.push(
          '图片URL:<input style="width:300px;" id="' +
            this.obj +
            '_txtImage" type="text"/><br/><span id="' +
            this.obj +
            '_spanImageErrorMessage"></span><br/>'
        );
        HTML.push(
          '<input value="插入图片" type="button" onclick="' +
            this.fullObjName +
            '.insertImage();"/>'
        );
        HTML.push("</div>");
    ...
    try {
      if (style === "width") {
        ...
        if (objWidth > this.config.container.maxWidth) {
          alert(
            "\u7f16\u8f91\u5668\u6700\u5927\u5bbd\u5ea6:" +
              this.config.container.maxWidth +
              "px"
          );
          ...
          if (objWidth < this.config.container.minWidth) {
            alert(
              "\u7f16\u8f91\u5668\u6700\u5c0f\u5bbd\u5ea6:" +
                this.config.container.minWidth +
                "px"
            );
        ...
        document.getElementById(this.config.container.id).style.width =
          obj.value;
        var editorToolBarTDWidth =
          parseInt(obj.value) - this.config.tool.convertHTML.ctrlWidth;
        if (this.config.tool.adjust.adjustable) {
          editorToolBarTDWidth =
            editorToolBarTDWidth - this.config.tool.adjust.width;
        }
        document.getElementById(this.config.tool.id).style.width = obj.value;
        document.getElementById(this.config.tool.id).style.width =
          editorToolBarTDWidth;
      } else {
        document.getElementById(this.config.container.id).style.height =
          obj.value;
      }
    } catch (ex) {
      if (style === "width") {
        obj.value = document.getElementById(
  ...
  Sparrow.editor.prototype.autoAdjust = function (obj) {
    var parentDivHeight = parseInt(
      document.getElementById(this.config.container.id).style.height
    );
    var toolBarHeight =
      document.getElementById(this.config.tool.id).clientHeight + 10;
  ...
  Sparrow.editor.prototype.convertHTML = function (obj) {
    ...
    if (obj.innerHTML === "HTML") {
      obj.innerHTML = "\u8fd4\u56de";
      obj.title = "\u8fd4\u56de\u6240\u89c1\u5373\u6240\u5f97\u6a21\u5f0f";
      document.getElementById(this.config.tool.icon.containerId).style.display =
        "none";
      if ($.browser.ie) {
        this.frame.contentWindow.document.body.innerText =
          this.frame.contentWindow.document.body.innerHTML;
  ...
  Sparrow.editor.prototype.insertVideo = function () {
    ...
    if (videoURL.trim() === "") {
      ...
      errorMessage.innerHTML =
        "请输入视频地址<br/>例:http://player.youku.com/player.php/sid/sparrow/v.swf";
    } else if (videoURL.search(/^[http:\/\/][^<]*\.swf[^<]*/) === -1) {
      errorMessage.className = "error";
      errorMessage.innerHTML =
        "请输入正确的视频地址。例:http://player.youku.com/player.php/sid/sparrow/v.swf";
    } else {
      ...
      if (this.config.flash_thumbnail) {
        ...
        $.ajax.json(
          $.url.root + "/attach/getFlashThumbnail.json",
          "parameter=" + encodeURIComponent(flashUrl),
          function (result) {
        ...
        videoHtml =
          '<embed src="{0}" quality="high" wmode="opaque" pluginspage="http://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash" width="500" height="500"/>'.format(
            flashUrl
          );
        if (tempNode) {
          tempNode.innerHTML = videoHtml;
  ...
  Sparrow.editor.prototype.insertImage = function () {
    ...
    if (imageURL.trim() === "") {
      ...
      var editor = this;
      $.ajax.json(
        $.url.root + "/attach/downloadInternetPic.json",
        "imageUrl=" + imageURL,
        function (result) {
          $(editor.config.attach.uploadImgContainerId).appendChild(
            getImgContainer(result.message, imageURL, editor)
          );
          editor.attach.insertEditor(
            $.file.getFileName(imageURL),
            result.message
          );
        }
      );
  ...
  Sparrow.editor.prototype.initialize = function (containerId) {
    ...
    document.getElementById(containerId).innerHTML =
      this.config.tool.position == "top"
        ? this.initTool() + this.initEditor(iframeId)
    ...
    this.frame.contentWindow.document.write(
      '<html><body id="' +
        this.obj +
        '" style="width:100%;height:{0}px;" contenteditable="true"></body></html>'.format(
          $(iframeId).offsetHeight
        )
    );
  ...
  Sparrow.editor.prototype.initTool = function () {
    ...
    if (this.config.style != null) {
      ...
      for (var i = 0; i < toolBarList.length; i++) {
        ...
        if (typeof this.config.tool.toolBar[key].split != "undefined") {
          toolHTML.push(
            this.config.tool.toolBar[key].split.format(
              this.config.tool.icon.path
            )
          );
        } else {
          ...
          toolHTML.push(
            this.config.tool.toolBar[key].left +
              ...
              this.config.tool.toolBar[key].top +
              "px;width:" +
              this.config.tool.toolBar[key].width +
              "px;height:" +
              this.config.tool.toolBar[key].height +
              'px"'
          );
          toolHTML.push(
            ' onmouseover="' +
              this.fullObjName +
              '.m_over(this);" onmouseout="' +
              this.fullObjName +
              '.m_out(this);"' +
              ' onmousedown="' +
              this.fullObjName +
              '.m_down(this);" onmouseup="' +
              this.fullObjName +
              '.m_up(this);"'
          );
          toolHTML.push("/>");
    ...
    if (this.config.tool.adjust.adjustable) {
      ...
      toolHTML.push(
        '<div align="center" title="\u5728\u6b64\u8c03\u6574\u7f16\u8f91\u5668\u5927\u5c0f\u3002\u9f20\u6807\u79bb\u5f00\u5373\u751f\u6548\u3002" ' +
    ...
    return (
      '<div class="pure-g tool-bar" id="' +
      this.config.tool.id +
      '" style="border-bottom:#ccc 1px solid; width:' +
      document.getElementById(this.config.container.id).style.width +
      ';height:auto;">' +
      toolHTML.join("") +
      "</div>"
  ...
  Sparrow.editor.prototype.initEditor = function (iframeId) {
    ...
    return (
      ...
      //-2为border
      (document.getElementById(this.config.container.id).offsetHeight -
        this.config.tool.height -
        2) +
      "px" +
  ...
  Sparrow.editor.prototype.updateWordCount = function () {
    ...
    if ($(editor.config.wordCount)) {
      $(editor.config.wordCount).innerHTML =
        editor.frame.contentWindow.document.body.innerHTML.length;
    }
  };
  ...
  function getImgContainer(fileUrl, clientFileName, editor) {
    ...
    imgDiv.s.className = "";
    imgDiv.s.style.cssText =
      "width:92px;height:90px;float:left;padding:3px;border:#ccc 2px solid;";
    imgArray.push(
      '<a target="_blank" href="{0}" title="{1}"><img style="width:91px;height:67px;" src="{2}"/></a>'.format(
        fileUrl,
    ...
    imgArray.push(
      '<br/><a href="javascript:void(0);" target="_self" onclick="' +
        editor.fullObjName +
  ...
  function clearHtmlFrame() {
    for (var i = 0; i < $.editor.editorArray.length; i++) {
      if ($.editor.editorArray[i].config.currentHtmlId) {
        document.getElementById(
          $.editor.editorArray[i].config.currentHtmlId
        ).style.display = "none";
    ...
    $.file.validateUploadFile = function (f, key, editor) {
      if (
        ...
        $.file.uploadCallBack = function (uploadProgress, editor) {
          ...
          if (clientFileName !== "") {
            $(editor.config.attach.uploadImgContainerId).appendChild(
              getImgContainer(uploadProgress.fileUrl, clientFileName, editor)
            );
            editor.attach.insertEditor(
              $.file.getFileName(clientFileName),
              uploadProgress.fileUrl
            );
          }
        };
        $.file.uploadDelegate(key, editor);
  ...
  Sparrow.tree.prototype = {
    ...
    addBusinessEntity: function (id, pid, name, url, title, businessEntity) {
      this.aNodes[this.aNodes.length] = new Sparrow.treeNode(
        id,
        pid,
        name,
        url,
    ...
    showOrder: function (e) {
      ...
      orderDiv.html(
        (
          '<span class="pure-menu-heading pure-menu-link pure-order-menu-heading" onclick="{0}.delete_click({1});{0}.clearFloatFrame();" >删除</span>' +
          ...
          ' <ul id="ulChildrenList" class="pure-menu-children"></ul></li></ul>'
        ).format(this.fullObjName, addressIndex)
      );
      ...
      orderDiv.css(
        "left",
        sparrowElement.getAbsoluteLeft() + srcObject.offsetWidth + "px"
      );
      orderDiv.css(
        "top",
        sparrowElement.getAbsoluteTop() + srcObject.offsetHeight + "px"
      );
      ...
      for (var i = 0; i < this.aNodes.length; i++) {
        ...
        index++;
        listHTML +=
          '<li class="pure-menu-item" onclick="{0}.order({1},{2})"><a href="#" class="pure-menu-link">第<span>{2}</span>位</a></li>'.format(
            this.fullObjName,
            addressIndex,
            index
          );
    ...
    node: function (node, nodeId) {
      ...
      if (this.config.treeNodeClass) {
        ...
        classNum +=
          this.obj.substring(0, 1).toUpperCase() + this.obj.substring(1);
      }
      ...
      str +=
        'id="node' + this.obj + nodeId + '"  >' + this.indent(node, nodeId);
      if (this.config.useIcons) {
        if (this.config.useLines) {
          ...
          node.iconOpen = node.icon = node._hasChild
            ? this.icon.nolinefolder
        ...
        if (this.config.useRootIcon || node.pid !== this.root.id) {
          str +=
            ...
            (this.config.showOrder
              ? ' onmouseover="' + this.fullObjName + '.showOrder(event)"'
              : "") +
            ' id="i' +
            this.obj +
            nodeId +
        ...
        if (
          (node.showCtrl && node.pid !== -1) ||
          (node.pid === -1 && this.userRootIcon === true)
        ) {
          if (this.config.useRadio) {
            str +=
              '<input style="line-height:15px;height:15px;border:0;" type="radio" name="iTreerdb" id="r{1}{0}" onclick="{1}.getRadioSelected({0});{1}.s({0});" value="{2}"/>'.format(
                nodeId,
                this.fullObjName,
                node.id
              );
          ...
          if (this.config.useCheckbox === true) {
            str +=
              '<input style="line-height:15px;height:15px;border:0;" type="checkbox" name="iTreecbx" id="c{1}{0}" onclick="{1}.selectCheckbox({0});" value="{2}"/>'.format(
                nodeId,
                this.fullObjName,
                node.id
              );
      ...
      if (node.url) {
        ...
        if (
          ...
          str +=
            ' onclick="javascript: ' + this.fullObjName + ".s(" + nodeId + ");";
        }
        str +=
          (this.config.usePlusMinusIcons
            ? ""
            : node._hasChild && node._parentNode.id !== -1
            ? this.fullObjName + ".o(" + nodeId + ")"
            : "") + '">';
      } else if (
        (!this.config.useFolderLinks || !node.url) &&
        node._hasChild &&
      ...
      str +=
        '<span id="ntext' + this.obj + nodeId + '">' + node.name + "</span>";
      ...
      if (node._hasChild) {
        str +=
          ...
          (this.root.id === node.pid || node._isOpened ? "block" : "none") +
          ';">';
        // 如果不是动态加载
    ...
    indent: function (node, nodeId) {
      ...
      if (!this.config.usePlusMinusIcons) {
        str +=
          '<img style="width:0;height:0;" id="j' + this.obj + nodeId + '"/>';
        return str;
      }
      ...
      if (node._hasChild) {
        ...
        if (!this.config.useLines) {
          ...
          str += node._isOpened
            ? node._lastOfSameLevel && this.config.useLines
    ...
    s: function (id) {
      ...
      if (this.selectedNodeIndex !== id) {
        if (this.selectedNodeIndex || this.selectedNodeIndex === 0) {
          // 将之前的选中节点置为普通结点状态
          eOld = document.getElementById(
            "s" + this.obj + this.selectedNodeIndex
          );
    ...
    nodeStatus: function (isOpen, nodeIndex, isLastNodeOfSameLevel) {
      ...
      if (this.config.usePlusMinusIcons) {
        ...
        if (this.config.useIcons) {
          eIcon.src = isOpen
            ? this.aNodes[nodeIndex].iconOpen
            : this.aNodes[nodeIndex].icon;
        }
        eJoin.src = this.config.useLines
          ? isOpen
            ? isLastNodeOfSameLevel
              ? this.icon.minusBottom
              : this.icon.minus
            : isLastNodeOfSameLevel
            ? this.icon.plusBottom
    ...
    selectCheckbox: function (nodeId) {
      ...
      if (node.selectChild == undefined || node.selectChild == true) {
        currentSelected = document.getElementById(
          "c" + this.obj + nodeId
        ).checked;
        if (currentSelected) {
          var cn = node;
    ...
    show: function (e, width, maxHeight) {
      ...
      if (
        $(this.config.floatTreeId).innerHTML.trim() == "" &&
      ...
      HTMLObject.style.top =
        sparrowElement.getAbsoluteTop() + srcObject.offsetHeight + "px";
      this.interval = window.setInterval(
        this.fullObjName + ".intervalShow(" + maxHeight + ")",
        10
      );
    },
    ...
    initCodeToolip: function (codePrefix, ajaxUrl) {
      var htmlEvents = (
        "$('#'+{0}.config.descTextBoxId).bind('onchange',function(){" +
        "if($({0}.config.descTextBoxId).value==''){" +
        "$({0}.config.descHiddenId).value='';" +
        "if(typeof({0}.config.valueTextBoxId)!='undefined')" +
        "{$({0}.config.valueTextBoxId).value='';$({0}.config.valueTextBoxId).readOnly='readonly';}}});"
      ).format(this.fullObjName);
      eval(htmlEvents);
      var treeObject = this;
      ...
      if (!ajaxUrl) ajaxUrl = $.url.root + "/code/load";
      this.dbs = function (nodeIndex) {
        ...
        if (this.config.descTextBoxId != null) {
          ...
          if (descCtrl.type === "text") {
            ...
            descCtrl.innerHTML = this.getAllNameOfNode(
              this.aNodes[nodeIndex],
              "/"
            );
          }
        }
  ...
  Sparrow.share = {
    config: {
      ...
      share: [
        ...
        {
          url:
            "http://sns.qzone.qq.com/cgi-bin/qzshare/cgi_qzshare_onekey?style=202&width=80&height=31&showcount=1&url={0}&title={1}&pics={2}&desc={3}&summary={4}&site=" +
            encodeURIComponent(
              $.browser.getCookie($.browser.cookie.website_name)
            ) +
            "&otype=share",
          position: {
            left: 160,
    ...
    init: function () {
      ...
      for (var j = 0; j < this.config.share.length; j++) {
        shareTemplateArray.push(
          '<a target="_blank" style="height:32px;width:32px;display:inline-block;background:url({0}) {1}px {2}px;" title="{3}" href="{4}"></a>'.format(
            this.config.icon,
      ...
      for (var i = shareArray.length - 1; i >= 0; i--) {
        ...
        shareArray[i].parentNode.innerHTML = shareTemplate.format(
